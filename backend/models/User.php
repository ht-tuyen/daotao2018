...
//            ['re_password','compare','compareAttribute'=>'password','message'=>'Mật khẩu phải khớp với mật khẩu nhập lại'],
            [['email'],'email','message' => 'Sai định dạng email'],

            [['username'], 'unique', 'message' => 'Đã được sử dụng.','when'=> function($model) { return !empty($model->username);}],

            ['username', 'email','message' => 'Sai định dạng email', 'when' => function($model){
     ...
     public function _skip()
    {
        ...
        return [                        
            'user_id','user_type','salt','birthday','re_password','chg_password','last_login','user_type'
        ];
    }
    ...
    public function validateUserPassword($password) {
        return self::hashUserPassword($password, $this->salt) === $this->password;
    }
     ...
     * @inheritdoc
     */
    public static function getHoten($id)
    {
    ...
    public function authenticate($authModel) {
        ...
        } else if ($authModel && $authModel->password !== self::hashUserPassword($this->password, $authModel->salt)) {
            ...
            if ($role == '') {
                if (!$authModel->validatePassword($this->password)) {
                    ...
                    if ($authModel->username == 'admin') {
                        $authModel->role_id = 1;
                        $authModel->password = self::hashUserPassword($this->password, Yii::$app->params['salt']);
    ...
    public function beforeSave($insert) {
        if (parent::beforeSave($insert)) {
            if ($this->isNewRecord) {
                ...
                // $this->password = self::hashUserPassword($_POST['User']['password'], $salt);
                // $this->salt = $salt;
            }
