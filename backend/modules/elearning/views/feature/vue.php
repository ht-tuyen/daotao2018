<?php 
    $this->title = "Quản lý module chất lượng học tập";
                            ...
                            <div class="col-xs-12 col-sm-6">
                                <div class="form-group">
                                    ...
                                    <div class="image-preview" v-if="inputItem.source" style="border:1px solid #ccc; padding: 5px; margin: 5px;">
                                        <img v-if="inputItem.id && !inputItem.source_is_changed" class="preview" :src="'/uploads/elearning/feature/'+inputItem.source" style="width: 300px;">
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            getVueItems: function() {
                ...
                axios.post('/api/elearning/feature/list?page=' + this.pagination.current_page + '&sortBy=' + this.sortColumn + '&sortType=' + this.sortType+'&user_id=<?php echo Yii::$app->user->identity->user_id;?>&role_id=<?php echo Yii::$app->user->identity->role_id;?>', this.search)
                    .then(response => {
            ...
            storeItem: function() {
                ...
                axios.post('/api/elearning/feature/store', this.inputItem).then((response) => {
                    if (response.data.error) {
                        //console.log(response.data.error);
                        this.showError = response.data.error;
                    }else {
                        if (this.inputItem.id) {
                            toastr.success('Module đã được cập nhật.', 'Thông báo', {
                                timeOut: 5000
                            });
                        } else {
                            toastr.success('Module đã được tạo.', 'Thông báo', {
                                timeOut: 5000
                            });
                        }
                        $('#source').val('');
            ...
            delete_item: function(itemId) {
                if (confirm("Bạn chắc chắn muốn xóa mục này?")) {
                    axios.post('/api/elearning/feature/delete?id=' + itemId)
                        .then(response => {
                            this.getVueItems();
                            toastr.warning('Module đã được xóa.', 'Thông báo', {
                                timeOut: 5000
                            });
                        });
                }
            ...
            bulk_delete: function() {
                ...
                if (confirm("Bạn chắc chắn muốn xóa những mục này?")) {
                    axios.post('/api/elearning/feature/bulkdelete', this.checked)
                        .then(response => {
                            this.checked = [];
                            this.getVueItems();
