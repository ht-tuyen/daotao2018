var kieuDongBox = '#kieu_dong_box',
    ...
    Order = function () {
        this.vatTax = 10;
        this.root_tab_elm = $('.orders-form');
        this.formula = {
            //tinh chi phi ra can
            'raCan': '( dai * rong * donGia)',
            //tinh chi phi ra phim
            'raPhim': '( dai * rong * soluong * soMau * donGia)',
            //tinh chi phi ra phim
            'raPhimTayCuoi': '( dai_tay_cuoi * rong_tay_cuoi * tayLe * soMau * donGia)',
            //tinh chi phi ra kem bia
            'raKemBia': '( soMau * donGia)',
            //tinh chi phi ra kem bia
            'raKemRuot': '((tayChan * soMau + tayLe * soMauTayCuoi) * donGia)',
        ...
        this.appendCustomerToDroplist = function (json) {
            $('select[name*="[customer_id]"] option:last-child').before(
                $('<option>').val(json.cus_id).html(json.name)
            );
            return this;
        };

        this.appendSupplierToDropList = function (select, json) {
            select.find('option:last-child').before(
                $('<option>').val(json.supplierid).html(json.name)
            );
            return this;
        };
        ...
        this.getSubOrderLength = function () {
            return $('.orders-form .nav.nav-tabs li').size();
        };
        ...
        this.fillGiayRuotThem = function (selectElm) {
            ...
            html.find('select, input').each(function () {
                ...
                } else if (this.nodeName === 'INPUT' && this.name.indexOf('InCungKhoGiay') !== -1) {
                    this.value = 1;
                    $(this).prop('checked', true);
                } else if (this.nodeName === 'INPUT' && this.name.indexOf('SoToBuHao') !== -1) {
                    this.value = first_tbody.find('input[name="' + this.name + '"]').val();
        ...
        this.fillGiaCong = function (selectElm, is_ruot) {
            ...
            $.ajaxQueue({
                ...
                'success': function (html) {
                    ...
                    $(html).find('select[name*="NhaCungCapId"]').each(function(){
                        $(this).select2({
                            allowClear: true,
                            language: "vi",
                            placeholder: "Chọn nhà cung cấp",
            ...
            if (product_id > 0 && change_size === true && is_update === 0) {
                ...
                if (product.KhoInBia_default) {
                    ...
                    if (product.KieuInTroBia_default) {
                        rootFrame.find(kieuInTroBia).val(product.KieuInTroBia_default);
                    }
                } else {
                    rootFrame.find(quyCachInBia).val('4/0');
                }
                ...
                if (product.KhoInRuot_default) {
                    ...
                    if (product.KieuInTroRuot_default) {
                        rootFrame.find(kieuInTroRuot).val(product.KieuInTroRuot_default);
                    }
                } else {
                    rootFrame.find(quyCachInRuot).val('1/1');
                    rootFrame.find(kieuInTroRuot).val(2);
                }
        ...
        this.timKhoGiayBia = function (selectElm, real) {
            ...
            if(giayInGiaTrucTiep === 1 && kieuInGiaTrucTiep === 1)
                return false;
        ...
        this.timKhoGiayRuot = function (selectElm, real, stt) {
            ...
            var _this = this,
                ...
                MauPha,
                HeSoMauPha = parseFloat(rootFrame.find("#he_so_mau_pha").val()),
                inChungKho = parseInt(rootFrame.find('input[name*="InCungKhoGiay' + stt + '"]:checked').val()),
                kieuHop = _this.getProductKieuHop(selectElm),
                kichThuocTai = parseFloat(rootFrame.find('input[name*="kich_thuoc_tai"]').val()),
                kichThuocCaiDay = parseFloat(rootFrame.find('input[name*="kich_thuoc_cai_day"]').val()),
                boiSongE = parseInt(rootFrame.find('input[name*="boi_song_e"]:checked').val()),
            ...
            if (rootFrame.find('input[name*="[GiayRuotSoToBuHaoThem' + stt + ']"]').length && rootFrame.find('input[name*="[GiayRuotSoToBuHaoThem' + stt + ']"]').hasClass('userChanged')) {
                buHaoTayCuoi = rootFrame.find('input[name*="[GiayRuotSoToBuHaoThem' + stt + ']"]').val();
        ...
        this.updateChiPhiTab = function (selectElm) {
            ...
            if(rootFrame.find('input[name*="[checkbox_hasnt_formula_kieu_in]"]').is(':checked')){
                this.tinhChiPhiInTrucTiep(rootFrame);
            }

            if(rootFrame.find('input[name*="[checkbox_hasnt_formula_giay_in]"]').is(':checked')){
                this.tinhChiPhiGiayIn(rootFrame);
            }
      ...
      this.tinhChiPhiXuatRa = function (selectElm) {
        ...
        this.updatePaper = function (elm) {
            ...
            if (name.indexOf('GiayBiaChatLieu') !== -1) {
                ...
                if (val > 0) {
                    $.ajaxQueue({
                        ...
                        'success': function (data) {
                            ...
                            tbodyObj.find(GiayRuotNhaCungCap.selector).prop('disabled', false);
                        }
                    });
                } else {
                ...
                if(val !== 'add_new' && val > 0) {
                    ...
                    if (val > 0) {
                        ...
                        if (supplier_json && supplier_json[val].don_gia_dinh_luong) {
                ...
                if(val !== 'add_new' && val > 0) {
                    ...
                    if (val > 0 && supplier_json && supplier_json[val].don_gia_dinh_luong) {
                        don_gia_dinh_luong = jQuery.parseJSON(supplier_json[val].don_gia_dinh_luong);
                        don_gia = don_gia_dinh_luong[chatLieuId][dinhluong];
                ...
                var don_gia_dinh_luong, don_gia = 0, chatLieuId = Number($(':selected', GiayBiaChatLieu).val()),
                    dinhluong = val, ncc_id = Number(rootFrame.find('select[name*="GiayBiaNhaCungCap"]').val());
                if (val > 0 && supplier_json && supplier_json[ncc_id].don_gia_dinh_luong) {
                    don_gia_dinh_luong = jQuery.parseJSON(supplier_json[ncc_id].don_gia_dinh_luong);
                ...
                var don_gia_dinh_luong, don_gia = 0,
                    chatLieuId = Number($(':selected', GiayRuotChatLieu).val()), dinhluong = val, ncc_id = Number(rootFrame.find('select[name*="GiayRuotNhaCungCap"]').val());
                if (val > 0 && supplier_json && supplier_json[ncc_id].don_gia_dinh_luong) {
                    don_gia_dinh_luong = jQuery.parseJSON(supplier_json[ncc_id].don_gia_dinh_luong);
                    if (don_gia_dinh_luong) {
                        don_gia = don_gia_dinh_luong[chatLieuId][dinhluong];
                ...
                var don_gia_dinh_luong, don_gia = 0,
                    chatLieuId = Number($(':selected', ToGacChatLieu).val()), dinhluong = val, ncc_id = rootFrame.find('select[name*="GiayRuotNhaCungCap"]').val();
                if (val > 0 && supplier_json && supplier_json[ncc_id].don_gia_dinh_luong) {
            ...
            } else if (name.indexOf('ToGacPrice') !== -1) {
                this.tinhChiPhiNVL();
                ...
                if (contents_json && contents_json[val]) {
        ...
        this.tinhChiPhiGiayBiaHasntFormula = function (elm) {
            ...
            var giaTruocThue = Math.round(eval(donGia * soLuong));
            var giaSauThue = Math.round(giaTruocThue + eval(donGia * soLuong * (phanTram / 100)));
        ...
        this.tinhChiPhiGiayRuotHasntFormula = function (elm) {
            ...
            rootFrame.find('tbody.tbody_giay_ruot_order').each(function () {
                ...
                donGia = _this.removeFormat(donGia);
                giaTruocThue = Math.round(eval(donGia * soLuong));
                giaSauThue = Math.round(giaTruocThue + eval(donGia * soLuong * (phanTram / 100)));
                giaTruocThue = Math.round(giaTruocThue);
      ...
      this.tinhChiPhiGiaCong = function (selectElm) {
        ...
        rootFrame.find('.gia-cong-item').each(function () {
          var sub_gia_cong_tbody = $(this), giayruot_number = '',
            ...
            $gia_cong_so_luong = sub_gia_cong_tbody.find('.gia_cong_so_luong'),
            $gia_cong_so_to_in = sub_gia_cong_tbody.find('.gia_cong_so_to_in'),
            $gia_cong_so_mat = sub_gia_cong_tbody.find('.gia_cong_so_mat'),
            $gia_cong_don_gia = sub_gia_cong_tbody.find('.gia_cong_don_gia'),
            $gia_cong_so_trang = sub_gia_cong_tbody.find('.gia_cong_so_trang'),
        ...
        this.tinhChiPhiInTest = function (rootFrame) {
            ...
            if (isInTest === 1) {
                ...
                if (isNaN(soLuong) || isNaN(donGia)) {
                    ...
                    chiPhiInTest = Math.round(eval(this.formula.inTestS));
                }
        ...
        this.tinhChiPhiNVL = function (selectElm) {
            ...
            rootFrame.find('.table_chi_phi_khac tbody tr').each(function () {
                if(!$(this).find('input[name*="quantity"]').hasClass('userChanged')) {
            ...
            $.ajaxQueue({
                ...
                'success': function (data) {
                    if (!$.isEmptyObject(data)) {
                        ...
                        rootFrame.find('input[name*="[type]"]').each(function () {
                        ...
                        if (!$.isEmptyObject(data.toGac)) {
                            ...
                            rootFrame.find('.chiPhiToGac').html(data.toGac.chiPhiGac).formatCurrency();
                            rootFrame.find('.chiPhiToGacVat').html(chiPhiGacVAT).formatCurrency();
                        ...
                        if (!$.isEmptyObject(data.biaCarton)) {
                            ...
                            rootFrame.find('.chiPhiBiaCarton').html(data.biaCarton.chiPhiBiaCarton).formatCurrency();
                            rootFrame.find('.chiPhiBiaCartonVat').html(chiPhiBiaCartonVAT).formatCurrency();
        ...
        this.tinhChiPhiInTrucTiep = function (rootFrame) {
            var donGia = rootFrame.find('input[name*="DonGiaInHasntFormula"]').val(),
                soLuong = rootFrame.find('input[name*="SoLuongInHasntFormula"]').val(), thanhTien,
                soSanPham = this.getProductAmount(rootFrame);
            donGia = Number(this.removeFormat(donGia));
            soLuong = Number(this.removeFormat(soLuong));
            if(soLuong === 0 && soSanPham > 0){
                rootFrame.find('input[name*="SoLuongInHasntFormula"]').val(soSanPham);
                soLuong = soSanPham;
                this.numberInput();
            }
            thanhTien = donGia * soLuong;
            rootFrame.find('input[name*="ChiPhiIn"]').val(0);
            if (!isNaN(thanhTien))
                rootFrame.find('input[name*="ChiPhiInBia"]').val(thanhTien);
        };
