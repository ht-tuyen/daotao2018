/**
 * Created by Cuong Vu on 04/07/2017.
 */
...
var OrderView = function () {
    ...
    this.vanChuyenChange = function (elm) {
        ...
        if (elm.name.indexOf('vanChuyenTan_Chuyen') != -1) {
            if (elm.value == Number(VCHUYEN_CHUYEN)) {
        ...
        } else if (type == Number(VCHUYEN_TRONG_LUONG)) {
...
//            $('.giaTriThuc', rootFrame).html(tongTriGia1).formatCurrency();
    ...
    this.tinhChiPhi1SanPham = function (rootFrame) {
        ...
        if (isNaN(soLuong)) {
            console.error('isNaN so_luong');
            return false;

        } else if (isNaN(tongGiaTri)) {
            console.error('isNaN tongGiaTri ');
            return false;
        }
    ...
    this.tinhLoiNhuan = function (elm) {
        ...
        if (($("#check_is_photo").val() != 1 || $("#check_is_photo").val() == undefined || $("#check_is_photo").val() == 'undefined') && banTinh == 1) {
            this.tinhChiPhiPhu();
        }
        ...
        if (elm.name.indexOf('tongChiPhiPhanTram') != -1 || elm.name.indexOf('phanTramQLDN') != -1) {
            ...
            console.log(tongChiPhiDonGiaSP, phanTram, tongChiPhiGiaBanSP);
...
//                $('input[name*="tongChiPhiGiaBanSP"][type="hidden"]', phanTramFrame).val(elm.value).toNumber();
            ...
            var chuaThanhToan_t = Math.round(tongChiPhiPhaiThu + hoaHong_GiaKhachGui * vat / 100) - daThanhToan;
            var chuaThanhToan = Math.round(hoaHong_GiaKhachGui + hoaHong_GiaKhachGui * vat / 100) - daThanhToan;
            ...
            if (elm.value.replace(/[^0-9]+/g, '') > 0) {
                $('input[name*="unpaid"][type="text"]', rootFrame).val(chuaThanhToan > 0 ? chuaThanhToan : 0).formatCurrency();
                $('input[name*="unpaid"][type="hidden"]', rootFrame).val(chuaThanhToan > 0 ? chuaThanhToan : 0);
                $(".tr_unpaid").show();
            } else {
            ...
            console.log(STATUS_DA_CHOT_HOP_DONG);
            if (status >= STATUS_DA_CHOT_HOP_DONG)
            krajeeDialog.alert('Khi thay đổi đơn giá này, quý khách vui lòng lưu đơn hàng trước khi tiến hành thanh toán');
        }
...
//                $('.tongChiPhiGiaBanSPView').val(tongChiPhiGiaBanSP).formatCurrency();
        ...
        if (isNaN(soLuong)) {
            console.log('isNaN soLuong ');
            return false;

        } else if (isNaN(tongChiPhiGiaBanSP)) {
            console.log('isNaN tongChiPhiGiaBanSP ');
            return false;
        }
        ...
        if (isNaN(tongThu)) {
            console.error('isNaN tongThu ');
            return false;

        } else if (isNaN(tongLoiNhuan)) {
            console.error('isNaN tongLoiNhuan ');
            return false;
        }
...
//            ODR.core.DoanhThuNV(frame);
    ...
    this.tinhHoaHongKhachHang = function (frame) {
        if (frame == undefined || frame == "undefined") {
            console.error('tinhHoaHongKhachHang() - frame is undefined ');
            return false;
        }
...
//            console.log(type);
        ...
        if (type == Number(HOA_HONG_KHACH_GUI_GIA)) {
            ...
            if (isNaN(hoaHongGiaKhachGui)) {
                console.warn('hoaHongGiaKhachGui isNaN ');
                return false;

            }
            if (isNaN(tongChiPhiPhaiThu)) {
                console.warn('tongChiPhiPhaiThu isNaN ');
                return false;

            }
            if (isNaN(hoaHongPhanTramDpThu)) {
                console.warn('hoaHongPhanTramDpThu isNaN ');
                return false;
...
//                    $('input.hoaHongTongPhaiThuTxt', frame).val(0);
            ...
            if (isNaN(hoaHongPhanTramChoKhach)) {
                console.warn('isNaN hoaHongPhanTramChoKhach ');
                return false;

            } else if (isNaN(tongChiPhiPhaiThu)) {
                console.warn('isNaN tongChiPhiPhaiThu ');
                return false;
            }
...
//            $('select.ordersExport', frame).attr('disabled', true);
        ...
        if (isNaN(daThanhToan)) {
            console.error('isNaN daThanhToan ');
            return false;

        }
        if (isNaN(tongChiPhiPhaiThu)) {
            console.error('isNaN tongChiPhiPhaiThu ');
            return false;

        }
        if (isNaN(vat)) {
            console.error(' isNaN vat ');
            return false;
        }
...
//            $('input.congNo_unpaid', frame).val(tongChiPhiPhaiThu - daThanhToan).formatCurrency();
    ...
    this.tinhSoTienNv = function (hhnv_item) {
        ...
        if (hhnv_item == undefined || hhnv_item == "undefined") {
            console.error('hhnv_item is undefined ');
            return false;
        }
...
//            console.log('m:'+m, hhnv_item);
    ...
    this.tinhPhanTramNv = function (hhnv_item) {
        ...
        if (hhnv_item == undefined || hhnv_item == "undefined") {
            console.warn('hhnv_item is undefined ');
            return false;
        }
        ...
        if (isNaN(cost)) {
            console.warn('isNaN cost ');
            return false;

        } else if (isNaN(tongThu)) {
            console.warn('isNaN tongThu ');
            return false;
        }
...
//        doc.on('change', '.table_hoa_hong_kh input.hoaHongGiaKhachGuiTxt', function(e) {
    ...
    doc.on('click', ".save_payment_ajax", function () {
        var form = $('#add-payment-form-')[0];
        var info_id = $(this).data('order_info_id');
    ...
    doc.on('change', ".amount_payment_popup", function (e) {
        var tong_thanh_toan = $(this).val();
        tong_thanh_toan = tong_thanh_toan.replace(/,/g, "");
        $(this).val(tong_thanh_toan).formatCurrency();
        var can_thanh_toan = $('#payment-amount_tt').val();
        can_thanh_toan = parseInt(can_thanh_toan.replace(/,/g, ""));
        if (parseInt(tong_thanh_toan) > parseInt(can_thanh_toan)) {
            krajeeDialog.confirm('Bạn đã nhập qúa số tiền cần thanh toán, bạn có chắc chắn muốn sử dụng số này?', function (result) {
                if (!result) {
                    $('.amount_payment_popup ').val(can_thanh_toan).formatCurrency();
                } else {
                    $('#payment-overbalance').val(parseInt(tong_thanh_toan) - parseInt(can_thanh_toan));
                }
    ...
    function update_overbalance() {
        var Payment_amount = $('#payment-amount'),
            Payment_balance_total = $('#payment-balance_total'),
            tong_can_thanh_toan = $('#tong_can_thanh_toan'),
            Payment_amount_tt = $('#payment-amount_tt'),
    ...
    doc.on('change', '.table_hoahong_nhanvien tbody select', function (e) {
        ...
        if (val > 0) {
            $('tbody select', table_hoahong_nhanvien).not(this).each(function () {
                ...
                if (val_ == val) {
                    krajeeDialog.alert('Đã có nhân viên <strong>' + $option_selected.prop('text') + '</strong> trong danh sách. Hãy chọn nhân viên khác');
...
//            order_view.updateHoahongNv(this);
                        ...
                        if ($(".list_export_" + id).val() != "") {
                            ...
                            if ($("#other_send_ncc_" + info_id).is(":visible")) {
                                if ($("#other_send_ncc_" + info_id).val() == '') {
                                    krajeeDialog.alert('Bạn phải chọn nhà cung cấp để xuất exel');
                                } else {
                                    if (val > 0 && info_id > 0) {
                            ...
                            krajeeDialog.alert('Bạn hãy chọn mục cần xuất excel');
                        }
                    } else {
                        krajeeDialog.alert("Đơn hàng chưa được lưu đầy đủ thông tin.<br>Vui lòng lưu đơn hàng trước khi sử dụng tính năng này");
                    }
                }
            })
            ;
    ...
    function loadEmailNCC(type, order_info) {
        $.ajax({
            ...
            success: function (data) {
                if (!data)
                    data = '<div class="email_list_div" style="margin-top: 5px"><input type="text" class="form-control other_send_mail_email_' + order_info + '" style="width: 450px; margin-bottom: 3px;" name="other_send_mail_email[]" placeholder="Nhập email"/> <a href="javascript:;" class="addLineEmail">[Thêm]</a></div>';
    ...
    doc.on('click', '.a_gui_mail', function () {
        ...
        if ($(".list_export_" + id).val() != "") {
            ...
            if (!email_validate) {
                krajeeDialog.alert('Vui lòng kiểm tra lại email cần gửi');
                return false;
            }
            else if (!other_send_mail_title || other_send_mail_title == undefined) {
                krajeeDialog.alert('Vui lòng kiểm tra tiêu đề gửi mail');
                return false;
            } else if (!other_send_mail_content || other_send_mail_content == undefined) {
                krajeeDialog.alert('Vui lòng kiểm tra nội dung gửi mail');
                return false;
            }
            ...
            if (val > 0 && info_id > 0) {
                ...
                $.ajax({
                    ...
                    'success': function (data) {
                        if (data == 'success') {
                            krajeeDialog.alert('Gửi mail thành công');
                            setTimeout(function () {
                                location.reload();
                            }, 2000);
                        } else if (data = 'error') {
            ...
            krajeeDialog.alert('Bạn hãy chọn mục cần gửi mail');
        }
    });
    ...
    doc.on('click', 'a.add_schedule', function () {
        ...
        if (full_access == 'true') {
            ...
            if ($('select[name*="schedule_title"]', parent_schedule).val() == '') {
                ...
                var clonerow = $('.template-schedule .schedule-row').first().clone();
                var show_all_schedule = $('.show_all_schedule');
                var colsNumber = show_all_schedule.find('tbody tr').length;
                if (user == null || user == '') {
                ...
                clonerow.find('td:first-child').text(colsNumber);

                clonerow.find('input, select').each(function () {
                    this.name = this.name.replace('[0]', '['+colsNumber+']');
                });

                clonerow.find('select[name*="Schedule['+colsNumber+'][title]"]').find('option[value="'+title+'"]').attr('selected', true);

                clonerow.find('.schedule_user_edit option').each(function () {
                    var $this = $(this);
                ...
                clonerow.find('.schedule_time_edit').val(time);
                clonerow.find('.schedule_time_done_edit').val(time_done);

                show_all_schedule.find('tbody').append(clonerow);
...
//                 });
    ...
    doc.on('click', '.show_all_schedule .remove_schedule', function () {
        $(this).closest('tr').remove();
    });
