...
class ActiveField extends Component
{
    ...
    {        
        $options['class'] = 'form-readonly form-control';
        // if($options['multiline'] == true) $options['class'] .= ' form-multiline';
        $options = array_merge($this->inputOptions, $options);
        $this->addAriaAttributes($options);
        $this->adjustLabelFor($options);      
        // print_r($this->model[$this->attribute]);
        $this->parts['{input}'] = Html::label($this->model[$this->attribute],null, ['class' => $options['class']]);
        return $this;
    }
